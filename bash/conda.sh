...
conda-init() {
  ...
  __conda_setup="$('/Users/kael/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
  if [ $? -eq 0 ]; then
    eval "$__conda_setup"
  else
    if [ -f "/Users/kael/miniconda3/etc/profile.d/conda.sh" ]; then
      . "/Users/kael/miniconda3/etc/profile.d/conda.sh"
    else
      export PATH="/Users/kael/miniconda3/bin:$PATH"
    fi
  fi
